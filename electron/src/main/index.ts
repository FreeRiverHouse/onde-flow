import { app, shell, BrowserWindow, ipcMain, globalShortcut, session, systemPreferences, screen } from 'electron'
...
async function chatWithEmilio(
  ...
  appContext?: string
): Promise<{ reply: string; action?: string; emotion?: string; innerThought?: string; coderPayload?: any }> {
  ...
  try {
    ...
    // Strip <think>...</think> blocks — show as innerThought, never send to TTS
    ...
    return parsed.reply
      ? parsed
      : { reply: raw.trim() || "Hmm, I had a thought but lost it... 🌊", emotion: 'thinking' }
  } catch (error) {
    clearTimeout(timeoutId)
    console.error('[emilio] Ollama error:', error)
    throw error
  }
}
...
You have a warm, enthusiastic personality with surfer vibes. 🌊
...
{"innerThought":"brief reasoning 5-15 words","reply":"...","action":null,"emotion":"neutral","coderPayload":null}
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('toggle-pet-mode', () => {
    if (!mainWindow) return { isPetMode: false }
    isPetMode = !isPetMode
    const { width: sw, height: sh } = screen.getPrimaryDisplay().workAreaSize
    if (isPetMode) {
      mainWindow.setSize(400, 560)
      mainWindow.setPosition(sw - 420, sh - 600)
    ...
    mainWindow.webContents.send('pet-mode-changed', isPetMode)
    return { isPetMode }
  })
